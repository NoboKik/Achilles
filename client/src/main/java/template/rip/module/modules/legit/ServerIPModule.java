...
public class ServerIPModule extends Module implements Renderable {
    ...
    public ServerIPModule(Category category, Description description, String name) {
    ...
    private String getAddress() {
        if (mc.getCurrentServerEntry() == null) return "Singleplayer";
        return mc.isIntegratedServerRunning() ? "Singleplayer" : mc.getCurrentServerEntry().address;
    }
    ...
    private void drawText() {
        ...
        if (invert.isEnabled()) {
            if (backgroundEnabled.isEnabled()) text = String.format("IP: %s", value);
            else text = String.format("[IP: %s]", value);
            title = "IP ";
        } else {
