...
public class PingModule extends Module implements Renderable {
    ...
    public PingModule(Category category, Description description, String name) {
    ...
    private long getPing() {
        if (mc.getNetworkHandler() != null && mc.player != null) {
            PlayerListEntry ple = mc.getNetworkHandler().getPlayerListEntry(mc.player.getUuid());
            if (ple != null) {
    ...
    private void drawText() {
        String text;
        String title;
        String value = getPing()+"";
        if (invert.isEnabled()) {
            if (backgroundEnabled.isEnabled()) text = String.format("Ping: %s", value);
            else text = String.format("[Ping: %s]", value);
            title = "Ping ";
        } else {
            if (backgroundEnabled.isEnabled()) text = String.format("%s ms", value);
            else text = String.format("[%s ms]", value);
            title = " ms";
        }
