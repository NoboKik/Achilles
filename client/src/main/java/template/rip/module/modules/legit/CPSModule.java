...
public class CPSModule extends Module implements Renderable {
    ...
    public final BooleanSetting rightClickEnabled = new BooleanSetting(this, true, "Right Click");
    ...
    public CPSModule(Category category, Description description, String name) {
    ...
    private void drawText() {
        ...
        String value = Template.cpsHelper().getCPS(0)+"";
        if (rightClickEnabled.isEnabled()) value = Template.cpsHelper().getCPS(0) + " | " + Template.cpsHelper().getCPS(1);

        if (invert.isEnabled()) {
            if (backgroundEnabled.isEnabled()) text = String.format("CPS: %s", value);
            else text = String.format("[CPS: %s]", value);
            title = "CPS ";
        } else {
            if (backgroundEnabled.isEnabled()) text = String.format("%s CPS", value);
            else text = String.format("[%s CPS]", value);
            title = " CPS";
        }
    ...
    @Override
    public void render() {
        if (!Template.displayRender() || Template.moduleManager == null || Template.moduleManager.getModule(this.getClass()) == null || !AchillesMenu.isClientEnabled())
            return;
