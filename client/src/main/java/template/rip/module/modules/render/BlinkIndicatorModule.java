...
public class BlinkIndicatorModule extends Module implements Renderable {
    ...
    public final NumberSetting distance = new NumberSetting(this, 70, 0, 500, 1, "Distance");
    ...
    public long lastEnable = 0L;
    public long lastDisable = 0L;
    public boolean lastToggle = false;

    public BlinkIndicatorModule(Category category, Description description, String name) {
    ...
    public String count() {
        if (Template.blinkUtil().blink) {
            return String.format("%.2f", Math.ceil((double) (System.currentTimeMillis() - lastEnable) / 50) * 0.05);
        } else {
            return String.format("%.2f", Math.ceil((double) (lastDisable - lastEnable) / 50) * 0.05);
        }
    }
    ...
    public void render() {
        ...
        if (Template.blinkUtil().blink && !lastToggle) lastEnable = System.currentTimeMillis();
        if (!Template.blinkUtil().blink && lastToggle) lastDisable = System.currentTimeMillis();
        lastToggle = Template.blinkUtil().blink;
        ...
        if (animation.isEnabled()) {
            if (Template.blinkUtil().blink) {
                long elapsed = System.currentTimeMillis() - lastEnable;
                progress = elapsed / duration.getFValue();
                if (progress > 1f) progress = 1f;
            } else {
                long elapsed = System.currentTimeMillis() - lastDisable;
                progress = 1f - (elapsed / duration.getFValue());
                if (progress < 0f) progress = 0f;
            }
            progress = EasingUtil.easeInSine(progress);
        } else {
            progress = Template.blinkUtil().blink ? 1 : 0;
        }
        ...
        String prefix = "blinking: ";
        if (!this.suffix.isEnabled()) prefix = "";
        ImGui.pushFont(font);

        String text = "blinking: "+count()+"s";
