...
public class ReachDisplayModule extends Module implements Renderable {

    //public final PosSetting position = new PosSetting(this, 20, 20);
    public final NumberSetting reachCap = new NumberSetting(this, 3, 0, 6, 0.1, "Reach Cap");
    public final NumberSetting decay = new NumberSetting(this, 2500, 500, 5000, 1, "Display decay");
    ...
    public ReachDisplayModule(Category category, Description description, String name) {
    ...
    @EventHandler
    public void onAttack(AttackEntityEvent.Pre e) {
        assert mc.player != null;
        Vec3d vec3 = mc.player.getCameraPosVec(1);
        blocks = Math.min(e.target.getEyePos().distanceTo(vec3), reachCap.getFValue());
        timer = System.currentTimeMillis() + decay.getIValue();
    }
    ...
    private long getTotems() {
        return ItemHelper.getTotal(new ItemStack(Items.TOTEM_OF_UNDYING));
    }
    ...
    private void drawText() {
        ...
        if (timer < System.currentTimeMillis()) {
            ...
            value = String.format("%s blocks", dF.format(blocks));
        if (invert.isEnabled()) {
            if (backgroundEnabled.isEnabled()) text = String.format("Reach: %s", value);
            else text = String.format("[Reach: %s]", value);
            title = "Reach ";
        } else {
            if (backgroundEnabled.isEnabled()) text = value;
            else text = String.format("[%s]", value);
            title = "";
        }
