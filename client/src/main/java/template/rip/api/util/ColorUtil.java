...
public class ColorUtil {
    ...
    public static Color interpolateColorsBackAndForth(int speed, int index, Color start, Color end, boolean trueColor) {
        long div = (System.currentTimeMillis()) / speed;
        if (speed == 11) div = 0;
        int angle = (int) ((div + index) % 360);
    ...
    public static JColor rainbow(int speed, int index, float saturation, float brightness, float opacity) {
        long div = (System.currentTimeMillis()) / speed;
        if (speed == 11) div = 0;
        int angle = (int) ((div + index) % 360);
        float hue = angle / 360f;
        Color color = new Color(Color.HSBtoRGB(hue, saturation, brightness));
        return new JColor(color.getRed(), color.getGreen(), color.getBlue(), Math.max(0, Math.min(255, (int) (opacity * 255))));
    }
